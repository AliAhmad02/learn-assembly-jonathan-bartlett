...
promptformat:
  .string "Enter two numbers separated by spaces, then press enter.\n"

retryprompt:
  .string "Press Y to try again or any other character to finish.\n"

scanformat:
  .string "%ld %ld"

resultformat1:
  .string "%ld is larger than %ld.\n"

resultformat2:
  .string "The numbers %ld and %ld are equal.\n"
...
main:
  ...
  # Get the user input for retrying into restart_input
  movq stdin, %rdi
  leaq scanretry, %rsi
  leaq restart_input, %rdx
  movq $0, %rax
  call fscanf

  # Move the user input into %rcx for comparison and destroy stack frame
  movb restart_input, %cl
  leave
 
  # Check if the user input was Y. If it was, retry. Otherwise, we're done
  cmpb $'Y', %cl
  je main
